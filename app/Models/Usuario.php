...
class Usuario {
     ...
     * @return bool
     */
    public function actualizarPerfil($usuario_id, $nombre_artistico, $biografia, $redes_sociales, $avatarFile = null, $bannerFile = null, $avatarUrl = null, $bannerUrl = null) {
        try {
                $this->db->beginTransaction();
                ...
                // Set default URLs if no image is provided and no existing image exists
                if (!$avatarFile && empty($avatarUrl)) {
                    $avatarUrl = CloudinaryConfig::getDefaultAvatarUrl();
                }
                if (!$bannerFile && empty($bannerUrl)) {
                ...
                if ($avatarFile && !empty($avatarFile['tmp_name'])) {
                    ...
                    if ($validacion['valid']) {
                        if (!empty($perfilActual['avatar_url']) && 
                            $perfilActual['avatar_url'] !== CloudinaryConfig::getDefaultAvatarUrl()) {
                ...
                if ($bannerFile && !empty($bannerFile['tmp_name'])) {
                    ...
                    if ($validacion['valid']) {
                        if (!empty($perfilActual['banner_url']) && 
                            $perfilActual['banner_url'] !== CloudinaryConfig::getDefaultBannerUrl()) {
